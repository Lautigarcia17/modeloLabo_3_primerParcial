...
class Usuario implements IBM
{
    ...
    public function Agregar() : bool
    {
        ...
        try {
            ...
            $sql = $pdo->prepare("INSERT INTO `usuarios`(`nombre`,`correo`, `clave`,`id_perfil`) VALUES (:nombre,:correo,:clave,:id_perfil)"); // retonar false o retorna un PDOStatement
    ...
    // public static function TraerUno($params) : Usuario  | bool   // EN BASE DE DATOS
    // {
    //     try 
    //     {
    ...
    public static function TraerUno($params) : Usuario  // EN JSON
    {
        $objetoRetorno = new Usuario("","","");
        if (file_exists("archivos/usuarios.json")) 
        {
            $datosParametro = json_decode($params);
            $archivo = fopen("archivos/usuarios.json","r");
            while(!feof($archivo))
            {
                $linea = fgets($archivo);
                ...
                {
                    $objeto = json_decode($linea); //json a objeto
                    if ($datosParametro->correo == $objeto->correo && $datosParametro->clave == $objeto->clave) {
                        $objetoRetorno = new Usuario($objeto->nombre,$objeto->correo,$objeto->clave);
                        break;
                    }
                }
            }
